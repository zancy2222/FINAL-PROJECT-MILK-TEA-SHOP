...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'addCategory') {
        ...
        if (!empty($_FILES['image']['name'])) {
            $targetDir = 'uploads/';
            $fileName = basename($_FILES['image']['name']); // Extract file name only
            $imagePath = $fileName; // Save only the file name in the database
            $fullPath = $targetDir . $fileName; // Full path for saving the file
        ...
        if (!empty($_FILES['image']['name'])) {
            $targetDir = 'uploads/';
            $fileName = basename($_FILES['image']['name']); // Extract only the file name
            $imagePath = $fileName; // Save only the file name in the database
            $fullPath = $targetDir . $fileName; // Full path for saving the file
            move_uploaded_file($_FILES['image']['tmp_name'], $fullPath);
